...
class WorldInteractionsValidator(WorldInteractionsValidatorProtocol):
    ...
    def is_position_allowed(self, position: Position, allowed_terrains: set[Terrain] = None) -> bool:
        occupied = self._world_state_service.is_occupied(position)
        reserved = self._world_state_service.is_reserved(position)
        ...
        return self._world_map.is_position_allowed(position, allowed_terrains)
